...
sub-types, prognosis and treatment response and risk of recurrence in breast 
cancer ([Parker et al., 2009; Symanns et al., 2011][References]) and currently 
feature as part of standard care and management of early stage estrogen-positive breast 
cancer. Although this package was original developed to assess ER and HER2 
pathway activity in breast cancer transcriptomic data sets, the package 
functionality is flexible and could be applied to transcriptomic data sets and 
gene signatures outside the context of breast cancer. 
...
and HER2 signatures built-in PathAnalyser can be accessed by typing the 
following in R:
```{r}
# PAM50 list object in PathAnalyser
data("pam50")
str(pam50)
# PAM50 ER signature
pam50$ER
# Pam50 HER2 signature
pam50$HER2
```
...
#### ER signature
The luminal A and B sub-types in breast cancer have been shown to be ER-positive,
therefore a list of the most informative genes for the PAM50-derived 
ER-signature were curated from these two specific sub-types. First the PAM50 
genes, were ordered by the luminal A centroid index in ascending order and the 
relative expression level represented by the centroid index were plotted to 
assist manual selection of the most informative genes (Fig. \@ref(fig:lumAPam50))
Genes that had most deviant expression levels relative the rest of the genes were 
selected as the most informative genes. Five genes: RMMD, BIRC5, MYBL2, CEP55, CDCA1
were selected as the down-regulated gene-set (coloured blue) and four genes 
(FOXA1, SLC39A6, ESR1, NAT1) were selected for the up-regulated gene-set of the 
PAM50 ER signature (coloured red).

```{r lumAPam50, echo=FALSE, fig.wide=TRUE, fig.cap="PAM50 genes ordered by their corresponding Luminal A centroid index."}
...
as the most informative genes for the sub-type (fig. \@ref(fig:lumBPam50)). An 
additional five genes (SFRP1, FOXC1, KRT17, MIA, CDH3) were selected for the 
down-regulated gene-set (coloured blue) for the PAM50 gene signature and the two 
genes (ESR1 and SLC39A6) which displayed the greatest up-regulation in the Luminal B 
sub-type were pre-selected for the up-regulated gene-set from the Luminal A 
gene-set curation (coloured red).
```{r lumBPam50, echo=FALSE, fig.wide=TRUE, fig.cap="PAM50 genes ordered by their corresponding Luminal B centroid index."}
...
created to assist the manual curation of the HER2 signature as shown in Figure 
\@ref(fig:her2Pam50). The most informative genes were selected which 
demonstrated the most deviant expression levels (coloured red and blue), with 
six genes (ESR1, SFRP1, MYC, FOXC1, MIA, SLC39A6) selected as the most 
down-regulated genes constituting the down-regulated gene-set (coloured blue) 
and six genes (RRM2, FGFR4, GRB7, TMEM45B, ERBB2) were selected as the most up-regulated 
gene-set (coloured red) for the PAM50 HER2 gene signature.
```{r her2Pam50, echo=FALSE, fig.wide=TRUE, fig.cap="PAM50 genes ordered by their corresponding HER2 centroid index."}
...
obtained from 60 individuals (cases) subsetted from a much larger data set of 
...
also displayed. 
```{r fig.wide=TRUE}
norm_data <- check_signature_vs_dataset(norm_data, ER_sig_df)
```

#### Multi-named genes
Typically found in microarray data sets, some genes may be given multiple gene 
symbols delimited with "///". PathAnalyser will not individually parse these 
genes and such genes with multiple symbols will be filtered out of the gene 
expression matrix.
...
abundance of the down-regulated gene-set and also in the top quartile for 
expression abundance in the up-regulated gene-set reveal a consensus in the 
evidence of expression inconsistency with both gene-set of the gene-signature 
...
classes_df <- classify_gsva_percent(norm_data, ER_sig_df)
```
...
classes_df <- classify_gsva_percent(norm_data, ER_sig_df, percent_thresh=50)
```
...
```{r}
classes_df_50p <- classify_gsva_percent(norm_data, ER_sig_df,
                                        percent_thresh=50)
```
...
```{r fig.wide=TRUE}
gsva_scores_dist(norm_data, ER_sig_df)
...
```{r}
classes_df <- classify_gsva_abs(norm_data, ER_sig_df,
...
abundance expression, the distribution of GSVA scores that constitute the 
"valley" i.e. the area between the two peaks (fig. \@ref(fig:thresholds)), 
represent samples that exhibit relatively weaker evidence of differential 
expression abundance relative to other samples and the algorithm would classify 
these samples as having "Uncertain" pathway activity. Because the peaks 
represent the mode GSVA scores (one for high and low expression abundance), 
...
plot <- gsva_scores_dist(norm_data, ER_sig_df)
...
Conversely, expanding the difference between the thresholds would increase the 
