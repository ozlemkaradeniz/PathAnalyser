...
assessing pathway activity in a transcriptomic data set using PathAnalyser. 
Successive sections will describe these steps in greater detail.

1. *Read input gene expression and signature data from text files* 
(a genes vs samples matrix, and a list of genes provided in the geneset format 
(.grp)) into gene expression matrix and gene signature data frame formats used 
by PathAnalyser (See [section 3][Data formats] for further details).
...
classifier. `transform_matrix` performs log CPM (counts per million) 
transformation on an gene expression matrix containing raw RNAseq counts for
each gene per sample, in addition to outputting a boxplot which provides a 
sanity check for transformation.
```{r eval=FALSE}
# Transforming matrix with log cpm transformation and sanity check of the transformation
res <- transform_matrix(data_se)
transformed_se <- log_cpm(data_se)
# split gene signature into up-regulated gene set and down-regulated gene set
...
# Data formats
The PathAnalyser classification algorithm requires two types of input data:

## Input files

1. *Gene expression matrix* - a text file containing a gene expression matrix, 
where rows represent genes and columns samples of a gene expression data set. 
The gene expression matrix text file can contain expression values from RNAseq or 
...
2. *Gene signature files* - a list of genes provided in the [gene set file format](https://software.broadinstitute.org/cancer/software/gsea/wiki/index.php/Data_formats#GRP:_Gene_set_file_format_.28.2A.grp.29) (using .grp file extension) for either the up-regulated or 
down-regulated gene set of the pathway signature. A plethora of pathway gene 
signatures in the gene set file format can be found at 
[MSigDB](https://www.gsea-msigdb.org/gsea/msigdb/index.jsp). The snapshot below
shows the first 30 lines of the up-regulated gene set of the ERBB2 (HER2) gene 
signature.  
...
format text files (ending in .grp). The user is expected to provide one 
...
[Symanns et al. (2011)][References]. The total number of genes that are 
up-regulated (denoted with an expression value of 1) and down-regulated (denoted 
with an expression value of -1) in this gene signature are 101 and 59 genes 
respectively.
```{r}
dim(ER_sig_df)
head(ER_sig_df)
...
# Example application: Assessing ER and HER2 pathway activity in a breast cancer RNAseq dataset
PathAnalyser
...
For the remainder of this demonstration, we will use the built-in gene expression data set matrix `ER_data_se1`. The transform_matrix function performs sanity check after the transformation of gene expression matrix with log cpm transformation. It displays the distribution of gene expression before and after transformation with the help of boxplots. The transform function returns the transformed matrix after log cpm transformation.
```{r}
transform_matrix(ER_data_se1)
normalized_se <- transform(ER_data_se1)
...
([Hänzelmann et al. 2013][References]). In comparison to available gene set methods,
GSVA has been . However, unlike GSVA, which does not 
