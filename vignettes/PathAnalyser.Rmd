...
#Checking the overlap of gene symbols betweeen matrix and gene signatures (optional step)

check_matrix_sig_overlap(data_se, sig_df)
...
### Transforming matrix
The raw counts in the gene expression matrix are transformed by
log cpm transformation and for the sanity check of the transformation, the function returns one boxplot representing
the distribution of counts before transformation and another
box plot representating distribution of counts after normalization

```{r eval=FALSE}
transformed_data_se <- transform_matrix(data_se)
```

### Checking overlap between gene signature and matrix symbols/IDs
To get an estimate how many symbols/IDs intersect between matrix
and signature data.
...
```{r plot, fig.cap="Density plots showing the distribution of GSVA scores for samples in logCPM-normalised `ER_data_se1` data set for the up-regulated (left) and down-regulated gene set (right) of the pathway signature using `visualise_GSVA` function."}
...
```{r}
confusion_matrix <- calculate_accuracy("~/PathAnalyser/raw_data/Sample_lable.txt", classes_df, "ER",
