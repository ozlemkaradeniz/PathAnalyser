...
abstract: >
  ...
  transcriptomic datasets, the pathway-based classification algorithm which is 
  gene signature and gene expression dataset independent, could be applied to 
...
# Introduction
PathAnalyser provides functionality for pathway-based classification of samples
in gene expression data sets using pathway gene signatures. The package houses a
comprehensive suite of functions that constitute the typical workflow for 
assessing pathway activity, and this functionality can be sub-divided into 5 
broad categories:

1.  Input functions for reading expression data and gene signatures
2.  Quality control and pre-processing of input data
3.  Classification of samples by pathway activity
4.  Visualization of sample classification
5.  Classification Evaluation (used only when true labels are available)

The features unique to PathAnalyser package are:

1) It takes into account multiple sample for the analysis. This is an addition to GSVA package features, where a single sample is taken into account for analysis.

2) It classifies the IDs/symbols into active and inactive pathway. This classification is based on the two matrices representating up and down regulated expression of genes respectively.The matrices consists of GSVA scores derived after GSVA analysis. The user can provide two threshold (relatively high and low gene expression) among each matrices, which classifies gene IDs/symbols into active and inactive pathway. In the end of the analysis, the package returns a prediction accuracy score and ROC curve for verification of the accuray of the classification.

3) The package also returns a PCA plot for visualization of the categories (Active/Inactive/Uncertain) the gene symbols/IDs have been classified into.
...
# Data formats
## Input files
The PathAnalyser classification functions require two types of input data:

1. *Gene expression matrix* - a tab-delimited text file containing a gene 
...
2. *Gene signature* - this data is provided as two text files containing 
...
[Symanns et al. (2011)][References], a list of genes discovered to be 
co-expressed with estrogen receptor alpha and predictive of distance-relapse 
free survival in patients treated with tamoxifen (a selective estrogen receptor 
modulator). The total number of genes that are up-regulated (denoted with an 
expression value of 1) and down-regulated (denoted with an expression value of 
-1) in this gene signature are 101 and 59 genes respectively.
```{r}
dim(ER_sig_df)
head(ER_sig_df)
...
The `read_input_file` function reads gene expression data from a file with an delimiter, the function checks the delimiter of the file and reads the file accordingly. It returns an ouput with gene symbols/IDs as the rownames and columns representating each sample IDs in the dataset.
```{r eval=FALSE}
data_se <- read_input_file("/Users/taniyapal/Documents/Group Project/TCGA_unannotated.txt")
...
## Gene signature data
Gene signature files can be read by the `read_signature_data`function. It returns a dataframe with signature IDs/symbols in the first column and their expression pattern is represented as up regulated or down regulated by +1 and -1 respectively.
```{r eval=FALSE}
...
For the remainder of this demonstration, we will use the built-in gene expression data set matrix `ER_data_se1`. The `log_cpm` function performs sanity check after the transformation of gene expression matrix with log cpm transformation. It displays the distribution of gene expression before and after transformation with the help of boxplots. The transform function returns the transformed matrix after log cpm transformation.
```{r}
normalized_se <- log_cpm(ER_data_se1)
...
sample-wise gene set enrichment method, Gene set variation analysis ([GSVA](https://www.bioconductor.org/packages/release/bioc/html/GSVA.html)). 
...
The diagram below summarises the classification algorithm employed by 
PathAnalyser to classify samples by pathway activity.
![Overview of algorithm](algorithm_diagram.png)
...
Here is a brief overview of the key steps constituting the standard workflow for 
assessing pathway activity in an RNAseq transcriptomic data set using 
PathAnalyser. Successive sections will describe these steps in greater detail.

To read a gene expression matrix text file (rows corresponding to genes and 
columns corresponding to samples) into a gene expression matrix for the 
classification functions, pass the gene expression dataset text file path to the 
`read_input_file` function (See [section 3][Data formats] for further details).

```{r}
# Load transcriptomic data set (gene expression matrix of samples)
data_se <- read_input_file("~/PathAnalyser/raw_data/Toy_dataset.txt")
```
Currently, the package only works with gene signatures provided as two separate gene
set files (file extension .grp) for the up-regulated gene set and down-regulated
gene set of the signature. These gene set files are passed to the 
`read_signature_data` to create a data frame containing a list of gene names and
their corresponding expression values (-1 for down-regulated and 1 for 
up-regulated genes).
...
```
2. *QC and pre-processing of expression and signature data.* The gene expression 
matrix must be normalised before passing the matrix to the PathAnalyser 
classifier. `log_cpm` performs log CPM (counts per million) transformation on an 
gene expression matrix containing raw RNAseq counts for each gene per sample, in 
addition to displaying a boxplot which provides a sanity check for 
transformation.
```{r eval=FALSE}
# Transforming matrix with log cpm transformation and sanity check of the transformation
transformed_se <- log_cpm(data_se)
up_genes_df <- sig_df[sig_df$expression == 1, ] # will be removed
down_genes_df <- sig_df[sig_df$expression == -1, ] # will be removed
data_se <- check_signature_vs_dataset(transformed_se, up_genes_df, down_genes_df)
```
3. *Classify samples based on pathway activity.* Samples in a gene expression 
matrix can be classified according to evidence of expression consistency with 
the up-regulated and down-regulated gene sets of GSVA, by using absolute 
thresholds for high and low expression for each gene set of the signature via 
`classify_GSVA_abs` or percentile threshold for high and low expression for each 
gene set of the signature via `classify_GSVA_percent` (default=25%). The 
algorithm is described in detail.
```{r eval=FALSE}
# classify samples using a percentile threshold (here 25% - quartile threshold
# is used as default
classes_df <- classify_GSVA_percent(sig_df, transformed_se)
# using absolute thresholds
classes_df_abs <- classify_GSVA_abs(sig_df, transformed_se, up_thresh.low = -0.2, 
                                    up_thresh.high = 0.2, dn_thresh.low = -0.2, 
                                    dn_thresh.high = 0.2)
```
4. *Visualize pathway activity*
```{r eval=FALSE}
classes_pca(transformed_se, classes_df, pathway_name = "ER")
```
5. *Assess the accuracy of the pathway-based classification (if pathway activity 
class labels are available)*
```{r eval=FALSE}
# read true pathway activity class labels for data set samples
true_labels_df <- read.table("~/PathAnalyser/raw_data/Sample_labels.txt", header=TRUE,sep = "\t")
# assess accuracy and generate confusion matrix for classification
confusion_matrix <- calculate_accuracy(true_labels_df, classes_df, pathway = "ER", display_statistics = T)
```
