...
data_se <- read_input_file("~/PathAnalyser/raw_data/toy_data.tsv")
dim(data_se)
...
be used to perform a log CPM transformation on the gene expression matrix. 
Furthermore, `log_cpm` confirms a the logCPM transformation has been performed correctly since the boxplots 
...
normalized_se <- log_cpm(data_se)
normalized_se <- check_signature_vs_dataset(normalized_se, ER_sig)
```

## Classify breast cancer samples based on ER pathway activity.
...
## Assess the accuracy of the pathway-based classification
```{r}
