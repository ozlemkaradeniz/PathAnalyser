...
## Input files
The PathAnalyser classification functions require two types of input files:
...
The `log_cpm` function performs sanity check after the transformation of gene 
expression matrix with log cpm transformation. It displays the distribution of 
gene expression before and after transformation with the help of boxplots. The 
transform function returns the transformed matrix after log cpm transformation.
```{r}
...
transcriptomic dataset, we will use PathAnalyser to classify samples in an RNA
seq dataset
