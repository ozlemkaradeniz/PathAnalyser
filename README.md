...
## Visualising GSVA scores distribution

As GSVA scores generated by the GSVA algorithm are used under the hood by PathAnalyser to assess expression abundance consistency of the samples for the up-regulated and down-regulated gene-sets of the gene signature (please read vignette for more detailed information), users may like to view the underlying distribution of GSVA scores for each direction-associated gene-set of the signature.

To view the GSVA score distribution of the samples in the expression data sets for the up-regulated and down-regulated gene-sets of the gene signature, for example to allow manual selection of absolute GSVA thresholds, use `gsva_scores_dist` function with the expression matrix (`expr_mat`) and signature data frame (`sig_df`) passed as arguments:

```{r}
gsva_scores_dist(expr_mat, sig_df)
```

## Pathway-based classification using absolute GSVA score thresholds

If users would like to use absolute classification thresholds rather than percentile thresholds for classification, users can use the \`classify_gsva_abs\` function with the gene expression matrix, gene signature and four mandatory thresholds:

-   up_thresh.high: high expression abundance threshold
...
For any questions, feature requests, bug reports or issues regarding this development version of PathAnalyser, please use the "[issues](https://github.com/ozlemkaradeniz/PathAnalyser/issues)" tab located at the top-left of the GitHub repository page.
